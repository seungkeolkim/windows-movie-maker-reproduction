...
이 디렉터리는 `W-06`~`W-11`의 독립 실행용 작업 명세를 보관한다. `W-06`과 `W-07`은
완료됐고 `W-08`~`W-10`도 후속 계약까지 구현됐다. `W-11`은 아직 구현되지 않았다. 각
...
| `W-10` | 복구와 백그라운드 작업 · 완료 | [w-10-recovery-and-background-jobs.md](w-10-recovery-and-background-jobs.md) | 자동 저장·복구·다시 연결·파형·프록시 |
| `W-11` | Windows 실행·설치 | [w-11-windows-launcher-and-installer.md](w-11-windows-launcher-and-installer.md) | 네이티브 런처와 설치·업데이트·제거 흐름 |

다음 작업은 `W-11`이다. 후속 작업은 선행 작업의 공개 계약을 재사용하며, 선행 기능을
