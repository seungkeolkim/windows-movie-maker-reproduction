...
현재는 실행 환경 스캐폴딩, MVP·1.0 화면 계약을 검증하는 인터랙티브 Qt 목업과 UI에서
분리된 W-01 프로젝트 코어까지 구현되어 있습니다. 프로젝트 코어는 정수 나노초 시간,
불변 미디어·클립·트랙 모델과 원자적인 명령 이력을 제공합니다. 목업은 여전히 고정 샘플
상태만 바꾸며 실제 미디어 재생, 프로젝트 파일 저장과 MP4 출력 로직은 아직 구현 전입니다.
...
| 미디어·편집·렌더링 서비스 | W-02 이후 실제 파일 분석, 재생과 출력은 시작 전 |
| 첫 사용 가능 버전 | 미배포 |
