...
코어, W-02 실제 미디어 보관함, W-03 프로젝트 저장·열기, W-04 타임라인 편집과 W-05 실제
미리 보기까지 구현되어
있습니다. 로컬 영상·사진·오디오를 분석해 고정 트랙에 추가하고, 클립 이동·삭제·분할·트리밍,
사진 길이와 영상 속도 변경을 실행 취소·다시 실행할 수 있습니다. 결과는 `schema_version`이
있는 UTF-8 JSON 파일에 원자적으로 저장됩니다. 영상·사진 프레임의 재생·탐색은 실제 FFmpeg
디코딩을 사용하며 오디오 믹싱과 MP4 출력은 아직 목업입니다.
...
프로젝트 값과 명령 이력으로 동작합니다. 미리 보기 재생, 음량·효과와 출력은 고정 상태를
사용하는 인터랙티브 목업입니다. 버전과 DLL 로딩만 확인하고 창을 열지 않으려면 다음을
실행합니다.
...
| 타임라인 편집 | W-04 고정 트랙 추가·이동·삭제·분할·트리밍·길이·속도와 명령 기반 undo/redo 구현 |
| 미리 보기 | W-05 실제 시간축, 영상·사진 FFmpeg 디코딩, 탐색·프레임 이동과 최신 요청 취소 구현 |
| 오디오·렌더링 | W-06 원본음·음악 믹싱이 다음 작업, MP4 출력은 W-07 후속 작업 |
| 첫 사용 가능 버전 | 미배포 |
