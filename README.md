...
코어와 W-02 실제 미디어 보관함까지 구현되어 있습니다. 로컬 영상·사진·오디오를 다중 선택하면
ffprobe 분석 결과와 원본 시간 정보를 명령 경계를 통해 프로젝트에 추가하고, 시각 미디어는
ffmpeg 썸네일을 표시합니다. 타임라인 편집, 재생, 프로젝트 파일 저장과 MP4 출력은 여전히
목업이며 실제 로직은 구현 전입니다.
...
않습니다. 타임라인 편집, 속성, 미리 보기, 프로젝트 저장과 출력은 고정 상태를 사용하는
인터랙티브 목업입니다. 버전과 DLL 로딩만 확인하고 창을 열지 않으려면 다음을 실행합니다.
...
| 편집·재생·렌더링 서비스 | W-03 이후 저장, 실제 타임라인, 재생과 출력은 시작 전 |
| 첫 사용 가능 버전 | 미배포 |
