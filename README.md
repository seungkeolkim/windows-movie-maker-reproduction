...
미디어 가져오기로 영상을 보관함에 등록한 다음, 영상 카드를 아래 **영상·사진** 트랙으로
끌어 놓습니다. 빈 트랙이나 기존 클립 사이에 놓을 수 있으며, 파란 삽입선이 추가할 위치를
표시합니다. 사진과 스토리보드에서도 같은 방식으로 추가할 수 있습니다. 보관함의 원본
항목은 유지되며, 추가한 클립은 `Ctrl+Z`로 실행 취소할 수 있습니다.

기존 **타임라인에 추가** 버튼과 더블클릭도 사용할 수 있습니다.

영상 클립은 시간 순서의 장면들을 필름 스트립으로 보여 줍니다. 아래쪽의 작은 제목은 폭이
좁으면 말줄임표로 줄이며, 클립 위에 약 2초 동안 마우스를 올려두면 전체 제목을 볼 수
있습니다. [로컬 샘플 영상 안내](samples/README.md)에 반복 테스트용 파일을 기록합니다.
