...
코어, W-02 실제 미디어 보관함과 W-03 프로젝트 저장·열기까지 구현되어 있습니다. 로컬
영상·사진·오디오를 분석해 프로젝트에 추가하고, `schema_version`이 있는 UTF-8 JSON 파일을
원자적으로 저장하고 다시 열 수 있습니다. 실제 타임라인 편집, 재생과 MP4 출력은 여전히
목업이며 실제 로직은 구현 전입니다.
...
않습니다. 프로젝트 새로 만들기·저장·다른 이름으로 저장·열기도 실제 JSON 파일로 동작합니다.
타임라인 편집, 속성, 미리 보기와 출력은 고정 상태를 사용하는 인터랙티브 목업입니다. 버전과
...
| 편집·재생·렌더링 서비스 | W-04 이후 실제 타임라인, 재생과 출력은 시작 전 |
| 첫 사용 가능 버전 | 미배포 |
