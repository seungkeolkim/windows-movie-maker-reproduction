...
`목업 프레임` 표시는 실제 파일이나 처리 결과로 오해하지 않게 계속 노출한다.
...
3. 끝까지 재생한 뒤 다시 재생한다.
4. 미리 보기 음소거를 켜고 끈다.
5. 1.0 이전·다음 프레임 버튼을 눌러 30fps 기준 목업 이동 안내를 확인한다.
...
- 프레임 이동은 약 33ms씩 움직이되 실제 프레임 정확도를 주장하지 않는다.
- 실제 영상이 아닌 색상과 시간 기반 `목업 프레임`임을 계속 표시한다.

자동 검증: `test_empty_preview_explains_next_action`와 컨트롤러 상태 테스트.
